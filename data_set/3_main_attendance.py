    ...
    for full_info in unique_students:
        ...
        try:
            ...
            if len(parts) >= 4:
                ...
                # Use update() to set student record by roll_no key
                ref_students.update({
                    roll: {
                        'name': name,
                        'roll_no': roll,
...
while True:
    ...
    landmarks = []
    if ENABLE_LIVENESS: landmarks = face_recognition.face_landmarks(rgb, locs)
    ...
    for i, (enc, loc) in enumerate(zip(encs, locs)):
        ...
        if ENABLE_LIVENESS and name != "Unknown":
            if i < len(landmarks):
                ...
                if ear < EYE_AR_THRESH: blink_counters[name] += 1
                else:
                    if blink_counters[name] >= EYE_AR_CONSEC_FRAMES: verified_real[name] = True
                    blink_counters[name] = 0
                
                if verified_real[name]: is_real = True; color = (0, 255, 0)
        else: is_real = True; color = (0, 255, 0)
